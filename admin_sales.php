...
$allowed = ['overview', 'leads', 'agents', 'targets', 'messages', 'agent'];
...
if (!isset($_GET['range']) && trim((string) ($_GET['from'] ?? '')) === '' && in_array($tab, ['overview', 'leads', 'agent'], true)) {
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($action === 'save_agent') {
        ...
        $done = sales_agent_delete((int) post('agent_id'));
        ...
        flash(empty($saved['ok']) ? ($saved['error'] ?? 'Failed') : 'Target saved.', empty($saved['ok']) ? 'err' : 'ok');
        ...
        flash(empty($done['ok']) ? ($done['error'] ?? 'Failed') : 'Rejected lead hidden from the list. Reports still count it.', empty($done['ok']) ? 'err' : 'ok');
...
if ($tab === 'agent' && $agentRow) {
    $agentStats = sales_stats($agentId, $from, $to);
    $agentSeries = sales_series($agentId, $from, $to);
    $agentTarget = sales_target_for($agentId);
    $agentLeads = sales_leads_query(['agent_id' => $agentId, 'from' => $from, 'to' => $to]);
}
...
<div class="page-head">
  <div>
    ...
    <p class="lede">Agents clock in, log businesses, and update follow-ups. You set targets, message them, and onboard interested clients.</p>
  </div>
  <div class="actions page-actions">
    <a class="btn" href="<?= h(url('admin_sales.php?tab=agents&new=1')) ?>"><?= icon('plus', 16) ?>New agent</a>
    <a class="btn ghost" href="<?= h(url('admin_sales.php?tab=targets&new=1')) ?>"><?= icon('flag', 16) ?>New target</a>
  </div>
</div>

<nav class="planner-tabs" aria-label="Sales sections">
  <a class="planner-tab<?= $tab === 'overview' ? ' is-on' : '' ?>" href="<?= h(url('admin_sales.php?tab=overview')) ?>"><?= icon('reports', 16) ?><span>Overview</span></a>
  <a class="planner-tab<?= $tab === 'leads' ? ' is-on' : '' ?>" href="<?= h(url('admin_sales.php?tab=leads')) ?>"><?= icon('clients', 16) ?><span>Leads</span></a>
...
<div class="stats">
  ...
  <div class="card stat"><?= icon('ban', 20) ?><span>Rejected</span><strong><?= (int) $overall['rejected'] ?></strong></div>
</div>
...
<div class="card">
  ...
  <div class="table-scroll">
    <table class="grid">
      <thead><tr><th>Agent</th><th class="right">Reach</th><th class="right">Interested</th><th class="right">Onboarded</th><th class="right">Rejected</th><th></th></tr></thead>
      <tbody>
        <?php foreach ($top as $row): ?>
          <tr>
            <td><?= h($row['name']) ?><div class="muted"><?= h($row['email']) ?></div></td>
            <td class="right mono"><?= (int) $row['reach'] ?></td>
            <td class="right mono"><?= (int) $row['interested'] ?></td>
            <td class="right mono"><?= (int) $row['sales'] ?></td>
            <td class="right mono"><?= (int) $row['rejected'] ?></td>
            <td class="row-actions"><a class="btn ghost sm" href="<?= h(url('admin_sales.php?tab=agent&agent=' . (int) $row['id'])) ?>">Open</a></td>
          </tr>
        <?php endforeach; ?>
      </tbody>
    </table>
...
<div class="card">
          ...
          <tr>
            ...
            <td class="row-actions">
              ...
              <?php if (($lead['status'] ?? '') === 'rejected'): ?>
                <form method="post" style="display:inline" onsubmit="return confirm('Hide this rejected lead from the list? Reports keep the count.');"><?= csrf_field() ?><input type="hidden" name="action" value="delete_lead"><input type="hidden" name="lead_id" value="<?= (int) $lead['id'] ?>"><button class="btn ghost sm" type="submit">Remove</button></form>
              <?php endif; ?>
            </td>
          </tr>
        <?php endforeach; ?>
  ...
  <?php if (!$leads): ?><p class="empty">No leads in this filter.</p><?php endif; ?>
</div>
  ...
  <div class="table-scroll">
    <table class="grid">
      ...
      <tbody>
        <?php foreach ($agents as $a): ?>
          <tr>
            <td><a href="<?= h(url('admin_sales.php?tab=agent&agent=' . (int) $a['id'])) ?>"><?= h($a['name']) ?></a></td>
            <td><?= h($a['email']) ?></td>
            ...
            <td class="row-actions">
              ...
              <form method="post" style="display:inline" onsubmit="return confirm('Delete this agent login?');"><?= csrf_field() ?><input type="hidden" name="agent_id" value="<?= (int) $a['id'] ?>"><input type="hidden" name="action" value="delete_agent"><button class="btn ghost sm" type="submit">Delete</button></form>
            </td>
          </tr>
        <?php endforeach; ?>
      </tbody>
...
if ($tab === 'agent' && $agentRow):
    $reachGoal = (int) ($agentTarget['reach_target'] ?? 0);
    $salesGoal = (int) ($agentTarget['sales_target'] ?? 0);
    $agentPending = sales_leads_query(['agent_id' => $agentId, 'follow_bucket' => 'due']);
    $agentFollowed = sales_leads_query(['agent_id' => $agentId, 'follow_bucket' => 'done', 'from' => $from, 'to' => $to]);
    ?>
<div class="page-head" style="margin-top:0">
  <div>
    <h2><?= h($agentRow['name']) ?></h2>
    <p class="lede"><?= h($agentRow['email']) ?> · <?= ($agentRow['status'] ?? '') === 'suspended' ? 'Suspended' : 'Live' ?></p>
  </div>
</div>
<div class="stats">
  <div class="card stat"><span>Reach</span><strong><?= (int) $agentStats['reach'] ?><?= $reachGoal ? ' / ' . $reachGoal : '' ?></strong></div>
  <div class="card stat"><span>Onboarded</span><strong><?= (int) $agentStats['onboarded'] ?><?= $salesGoal ? ' / ' . $salesGoal : '' ?></strong></div>
  <div class="card stat"><span>Interested</span><strong><?= (int) $agentStats['interested'] ?></strong></div>
  <div class="card stat"><span>Follow up</span><strong><?= (int) $agentStats['follow_up'] ?></strong></div>
</div>
<?php if ($reachGoal || $salesGoal): ?>
<div class="card" style="margin-bottom:16px"><div class="pad-form">
  <?php if ($reachGoal): ?><p class="lede">Reach <?= (int) $agentStats['reach'] ?> / <?= $reachGoal ?></p><div class="savings-bar"><span style="width:<?= min(100, (int) round(100 * $agentStats['reach'] / max(1, $reachGoal))) ?>%"></span></div><?php endif; ?>
  <?php if ($salesGoal): ?><p class="lede" style="margin-top:12px">Sales <?= (int) $agentStats['onboarded'] ?> / <?= $salesGoal ?></p><div class="savings-bar"><span style="width:<?= min(100, (int) round(100 * $agentStats['onboarded'] / max(1, $salesGoal))) ?>%"></span></div><?php endif; ?>
</div></div>
<?php endif; ?>
<div class="chart-grid equal" style="margin-bottom:16px">
  <div class="card chart-box"><div class="card-head"><h2>Mix</h2></div><div class="pad-form" style="height:200px"><canvas id="agent-pie"></canvas></div></div>
  <div class="card chart-box"><div class="card-head"><h2>Time series</h2></div><div class="pad-form" style="height:200px"><canvas id="agent-line"></canvas></div></div>
</div>
...
layout_end('<script src="' . h(asset('js/chart.umd.min.js')) . '"></script><script>(function(){var d=' . $payload . ';var p=document.getElementById("agent-pie");if(p&&window.Chart)new Chart(p,{type:"doughnut",data:{labels:d.pieLabels,datasets:[{data:d.pieValues,backgroundColor:[d.color,"#c4a35a","#b42318","#0f766e"],borderWidth:0}]},options:{cutout:"58%",plugins:{legend:{position:"bottom"}},maintainAspectRatio:false}});var l=document.getElementById("agent-line");if(l&&window.Chart)new Chart(l,{type:"line",data:{labels:d.labels,datasets:[{data:d.reach,borderColor:d.color,tension:.3,fill:false}]},options:{plugins:{legend:{display:false}},scales:{y:{beginAtZero:true}},maintainAspectRatio:false}});})();</script>');
return;
endif;

if ($tab === 'targets'): ?>
<div class="card" style="margin-bottom:16px">
  <div class="card-head"><h2><?= icon('plus', 16) ?>Set target</h2></div>
  <form method="post" class="pad-form form-grid">
    <?= csrf_field() ?>
    ...
    <div><label>Sales target (onboarded)</label><input type="number" name="sales_target" min="0" value="2"></div>
    <div class="full"><label>Notes</label><input name="notes"></div>
    <div class="actions" style="grid-column:1/-1"><button class="btn" type="submit">Save target</button></div>
  </form>
</div>
<div class="card">
